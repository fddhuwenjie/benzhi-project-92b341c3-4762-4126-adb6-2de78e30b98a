...
type FileRepository struct {
	...
	idemGuard sync.Mutex
	idemCache map[string]IdempotencyRecord
}
...
func NewFileRepository(root string) (*FileRepository, error) {
	...
	r := &FileRepository{root: root, locks: map[string]*sync.Mutex{}, idemCache: map[string]IdempotencyRecord{}}
...
func (r *FileRepository) cachedIdempotency(requestID, fingerprint string) (CommitResult, bool, error) {
	r.idemGuard.Lock()
	defer r.idemGuard.Unlock()
	prior, ok := r.idemCache[requestID]
	if !ok {
		return CommitResult{}, false, nil
	}
	if prior.Fingerprint != fingerprint {
		return CommitResult{}, true, domain.NewError(domain.CodeConflict, "request_id 已用于不同请求")
	}
	return CommitResult{Response: prior.Response, Revision: prior.Revision, Replayed: true}, true, nil
}

func (r *FileRepository) rememberIdempotency(record IdempotencyRecord) {
	r.idemGuard.Lock()
	defer r.idemGuard.Unlock()
	r.idemCache[record.RequestID] = record
}
...
func (r *FileRepository) Execute(id, requestID, fingerprint string, expected uint64, create bool, mutation Mutation) (CommitResult, error) {
	...
	lock.Lock()
	defer lock.Unlock()
	if cached, ok, err := r.cachedIdempotency(requestID, fingerprint); ok {
		return cached, err
	}
