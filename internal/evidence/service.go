...
func New(repository store.Repository) *Service { return &Service{repository: repository} }

// validateCertificateEnvelope checks fields required before canonical verification.
func validateCertificateEnvelope(batchID string, cert *domain.ReleaseCertificate) error {
	if cert == nil {
		return domain.NewError(domain.CodeInvalid, "资格证书内容为空")
	}
	if cert.CertificateID == "" || cert.CertificateID != "CERT-"+batchID || cert.ReviewDecision != domain.DecisionApproved {
		return domain.NewError(domain.CodeInvalid, "资格证书封存字段无效")
	}
	return nil
}
...
func (s *Service) Download(batchID string) ([]byte, *domain.ReleaseCertificate, error) {
	...
	cert, err := s.repository.LoadCertificate(batchID)
	if err != nil {
		return nil, nil, err
	}
...
func (s *Service) Verify(batchID, claimedDigest string) (Verification, error) {
	...
	if err := validateCertificateEnvelope(batchID, cert); err != nil {
		return Verification{}, err
	}
	data, err := canonicalBytes(batch, *cert)
	if err != nil {
