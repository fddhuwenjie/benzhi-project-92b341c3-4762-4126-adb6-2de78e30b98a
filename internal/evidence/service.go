...
func New(repository store.Repository) *Service {
	return &Service{repository: repository, verificationCache: map[string]Verification{}}
}
...
func (s *Service) Verify(batchID, claimedDigest string) (Verification, error) {
	...
	cacheKey := batchID + "\x00" + claimedDigest
	s.verificationMu.RLock()
	cached, ok := s.verificationCache[cacheKey]
	s.verificationMu.RUnlock()
	if ok {
		return cloneVerification(cached), nil
	}
	batch, err := s.repository.Get(batchID)
	if err != nil {
	...
	result := Verification{Valid: valid, Message: message, Certificate: cert, Checks: checks}
	if valid {
		s.verificationMu.Lock()
		s.verificationCache[cacheKey] = cloneVerification(result)
