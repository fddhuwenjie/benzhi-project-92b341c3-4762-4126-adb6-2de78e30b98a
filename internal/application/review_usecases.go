...
func (s *Service) PreflightReviewer(batchID, reviewerID string) (domain.ReviewerEligibility, error) {
	...
	key := batchID + "\x00" + reviewerID
	s.reviewerCacheMu.RLock()
	cached, ok := s.reviewerCache[key]
	s.reviewerCacheMu.RUnlock()
	if ok {
		cached.Conflicts = append([]domain.ReviewerConflictFact(nil), cached.Conflicts...)
		return cached, nil
	}
	b, err := s.repo.Get(batchID)
	if err != nil {
	...
	result, err := b.ReviewerPreflight(reviewerID)
	if err != nil {
		return domain.ReviewerEligibility{}, err
	}
	stored := result
	stored.Conflicts = append([]domain.ReviewerConflictFact(nil), result.Conflicts...)
	s.reviewerCacheMu.Lock()
	s.reviewerCache[key] = stored
	s.reviewerCacheMu.Unlock()
	result.Conflicts = append([]domain.ReviewerConflictFact(nil), result.Conflicts...)
	return result, nil
}
