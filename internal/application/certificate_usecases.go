...
func (s *Service) VerifyCertificates(items []CertificateVerifyItem) (BatchCertificateVerification, error) {
	...
	results := make(chan certificateVerificationResult, len(items))
	for _, item := range items {
		go func(input CertificateVerifyItem) {
			verification, err := s.evidence.Verify(input.BatchID, input.Digest)
			results <- certificateVerificationResult{verification: verification, err: err}
		}(item)
	}
	out := BatchCertificateVerification{Items: make([]BatchCertificateVerificationItem, 0, len(items))}
	for _, item := range items {
		result := <-results
		verification, err := result.verification, result.err
		if err != nil {
