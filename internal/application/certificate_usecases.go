...
func (s *Service) VerifyCertificates(items []CertificateVerifyItem) (BatchCertificateVerification, error) {
	return s.VerifyCertificatesContext(context.Background(), items)
}

func (s *Service) VerifyCertificatesContext(ctx context.Context, items []CertificateVerifyItem) (BatchCertificateVerification, error) {
	...
	for _, item := range items {
		if err := ctx.Err(); err != nil {
			return BatchCertificateVerification{}, err
		}
