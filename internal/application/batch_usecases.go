...
func (s *Service) ReviseBatchInfo(batchID string, cmd ReviseBatchInfoCommand) (CommandResponse, error) {
	return s.reviseBatchInfo(context.Background(), batchID, cmd)
}

func (s *Service) ReviseBatchInfoContext(ctx context.Context, batchID string, cmd ReviseBatchInfoCommand) (CommandResponse, error) {
	return s.reviseBatchInfo(ctx, batchID, cmd)
}

func (s *Service) reviseBatchInfo(ctx context.Context, batchID string, cmd ReviseBatchInfoCommand) (CommandResponse, error) {
	return s.executeContext(ctx, batchID, cmd.CommandMeta, cmd, false, func(b *domain.QualificationBatch) (any, store.EventDraft, error) {
		now := s.clock()
