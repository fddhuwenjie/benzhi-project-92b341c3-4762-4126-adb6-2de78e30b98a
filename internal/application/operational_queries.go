...
func (s *Service) QueryMeasurementQuality(batchID string, filter MeasurementQualityFilter) (domain.MeasurementQualitySnapshot, error) {
	...
	cacheKey := fmt.Sprintf("%s|%s|%s|%s|%s", batchID, filter.TapID, filter.SurfaceZone, filter.RoundKind, filter.Level)
	s.measurementQualityMu.RLock()
	cached, found := s.measurementQualityCache[cacheKey]
	s.measurementQualityMu.RUnlock()
	if found {
		return cloneMeasurementQualitySnapshot(cached)
	}
	snapshot := b.MeasurementQualitySnapshot(domain.MeasurementQualityFilter{TapID: filter.TapID, SurfaceZone: filter.SurfaceZone, RoundKind: filter.RoundKind, Level: filter.Level}, s.clock())
	s.measurementQualityMu.Lock()
	s.measurementQualityCache[cacheKey] = snapshot
	s.measurementQualityMu.Unlock()
	return cloneMeasurementQualitySnapshot(snapshot)
}

func cloneMeasurementQualitySnapshot(snapshot domain.MeasurementQualitySnapshot) (domain.MeasurementQualitySnapshot, error) {
	data, err := json.Marshal(snapshot)
	if err != nil {
		return domain.MeasurementQualitySnapshot{}, err
	}
	var clone domain.MeasurementQualitySnapshot
