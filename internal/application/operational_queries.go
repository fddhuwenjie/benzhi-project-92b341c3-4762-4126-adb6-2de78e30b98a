...
func (s *Service) QueryAudit(batchID string, filter AuditFilter) (AuditPage, error) {
	...
	cacheKeyData, err := json.Marshal(struct {
		BatchID string      `json:"batch_id"`
		Filter  AuditFilter `json:"filter"`
	}{BatchID: batchID, Filter: filter})
	if err != nil {
	...
	page := AuditPage{BatchID: batchID, Filter: filter, Events: events, Total: total, Page: filter.Page, PageSize: filter.PageSize, Validation: evidence.ValidateAuditSegment(b.Audit, events)}
	encoded, err := json.Marshal(page)
	if err != nil {
		return AuditPage{}, err
	}
	s.auditMu.Lock()
	s.auditCache[cacheKey] = encoded
	s.auditMu.Unlock()
	return page, nil
}
