...
type defectTaskCacheKey struct {
	BatchID      string
	Revision     uint64
	TechnicianID string
...
func newDefectTaskCacheKey(batchID string, revision uint64, filter DefectTaskFilter) defectTaskCacheKey {
	key := defectTaskCacheKey{BatchID: batchID, Revision: revision, TechnicianID: filter.TechnicianID, Priority: filter.Priority, TaskStatus: filter.TaskStatus}
	if filter.DueAfter != nil {
		key.DueAfter = filter.DueAfter.UTC()
	}
	if filter.DueBefore != nil {
		key.DueBefore = filter.DueBefore.UTC()
	}
	return key
}
...
func (s *Service) QueryDefectTasks(batchID string, filter DefectTaskFilter) ([]DefectTaskView, error) {
	...
	cacheKey := newDefectTaskCacheKey(batchID, b.Revision, filter)
	s.defectTaskCacheMu.RLock()
	if cacheKey == s.defectTaskCacheKey && s.defectTaskCache != nil {
		cached := append([]byte(nil), s.defectTaskCache...)
		s.defectTaskCacheMu.RUnlock()
		var out []DefectTaskView
	...
	s.defectTaskCacheMu.Lock()
	s.defectTaskCacheKey = cacheKey
	s.defectTaskCache = append(s.defectTaskCache[:0], encoded...)
	s.defectTaskCacheMu.Unlock()
	return out, nil
}
