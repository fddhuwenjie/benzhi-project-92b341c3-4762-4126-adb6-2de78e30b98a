...
func (s *Service) QueryRevisionHistory(batchID string, filter RevisionHistoryFilter) (domain.RevisionHistoryView, error) {
	cacheKey := fmt.Sprintf("%s|%s|%d|%d", batchID, filter.HistoryType, filter.FromVersion, filter.ToVersion)
	s.historyMu.Lock()
	if cached, ok := s.historyCache[cacheKey]; ok {
		s.historyMu.Unlock()
		return cached, nil
	}
	s.historyMu.Unlock()
	...
	view, err := b.RevisionHistory(filter.HistoryType, filter.FromVersion, filter.ToVersion)
	if err != nil {
		return domain.RevisionHistoryView{}, err
	}
	s.historyMu.Lock()
	s.historyCache[cacheKey] = view
	s.historyMu.Unlock()
	return view, nil
}
