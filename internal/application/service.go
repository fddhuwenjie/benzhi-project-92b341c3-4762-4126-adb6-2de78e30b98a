...
type Service struct {
	...
	clock                   Clock
	measurementQualityMu    sync.RWMutex
	measurementQualityCache map[string]domain.MeasurementQualitySnapshot
}

func New(repo store.Repository, evidenceService *evidence.Service) *Service {
	return &Service{repo: repo, evidence: evidenceService, clock: func() time.Time { return time.Now().UTC() }, measurementQualityCache: map[string]domain.MeasurementQualitySnapshot{}}
}
