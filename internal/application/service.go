...
type Service struct {
	...
	auditCache map[string]json.RawMessage
}

func New(repo store.Repository, evidenceService *evidence.Service) *Service {
	return &Service{repo: repo, evidence: evidenceService, clock: func() time.Time { return time.Now().UTC() }, auditCache: map[string]json.RawMessage{}}
}
