...
type submissionPreflightCacheKey struct {
	batchID  string
	revision uint64
}
...
type Service struct {
	...
	clock                    Clock
	submissionPreflightMu    sync.Mutex
	submissionPreflightCache map[submissionPreflightCacheKey]domain.SubmissionPreflight
}
