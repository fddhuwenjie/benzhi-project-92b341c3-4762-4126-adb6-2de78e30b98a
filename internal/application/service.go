...
type draftViewCacheKey struct {
	batchID            string
	revision           uint64
	surfaceZone        string
	qualificationState domain.QualificationStatus
...
type Service struct {
	...
	clock               Clock
	draftViewCacheMu    sync.Mutex
	draftViewCacheKey   draftViewCacheKey
	draftViewCache      BatchView
	draftViewCacheValid bool
}
...
func (s *Service) GetBatchFiltered(id string, filter BatchFilter) (BatchView, error) {
	...
	key := draftBatchViewKey(b, filter)
	s.draftViewCacheMu.Lock()
	if s.draftViewCacheValid && s.draftViewCacheKey == key {
		cached := s.draftViewCache
		s.draftViewCacheMu.Unlock()
		return cached, nil
	}
	s.draftViewCacheMu.Unlock()

	view := projectBatchFiltered(b, cert, s.clock(), filter)
	s.draftViewCacheMu.Lock()
	s.draftViewCacheKey = key
	s.draftViewCache = view
	s.draftViewCacheValid = true
	s.draftViewCacheMu.Unlock()
	return view, nil
}

func draftBatchViewKey(batch *domain.QualificationBatch, filter BatchFilter) draftViewCacheKey {
	key := draftViewCacheKey{batchID: batch.BatchID, revision: batch.Revision, surfaceZone: filter.SurfaceZone, qualificationState: filter.QualificationStatus, defectType: filter.DefectType}
	if filter.Blocking != nil {
		key.blocking = *filter.Blocking
		key.hasBlocking = true
	}
	return key
}
