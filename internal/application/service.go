...
type Service struct {
	...
	historyCache map[string]domain.RevisionHistoryView
}

func New(repo store.Repository, evidenceService *evidence.Service) *Service {
	return &Service{repo: repo, evidence: evidenceService, clock: func() time.Time { return time.Now().UTC() }, historyCache: map[string]domain.RevisionHistoryView{}}
}
